  ...
  jQuery.fn.bt = function(content, options) {
    ...
    return this.each(function(index) {
      ...
      this.btOn = function () {
        ...
        if (opts.positions[0] == 'most' || opts.positions[0] == 'center') {
        ...
        if (opts.overlay) {
			...
			var overlay = $('<div class="bt-overlay"></div>').css({
				...
				height: $('body').height(),
				opacity: 0,
				zIndex: opts.boxzIndex-1
			}).appendTo($('body'));
			overlay.fadeTo(1, opts.overlayOpacity);
			$(this).data('overlay', overlay);
			
			if(opts.positions[0] == 'center'){
				var t = ($(window).height() - $(canvas).height() ) / 2 + $(window).scrollTop();
				var l = ($(window).width() - $(canvas).width() ) / 2 - $(window).scrollLeft();
				$box.css('top', t-50);
				$box.css('left', l-50);
				$('body').append($box);
				$(canvas).css("top", 0);
        ...
        $(canvas).attr('width', (numb($text.btOuterWidth(true)) + opts.strokeWidth*2 + shadowMarginX)).attr('height', (numb($text.outerHeight(true)) + opts.strokeWidth*2 + shadowMarginY)).css({position: 'absolute', zIndex: opts.boxzIndex});
